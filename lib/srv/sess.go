...
func (s *session) trackerCreate(teleportUser string, policySet []*types.SessionTrackerPolicySet) error {
	...
	err = s.registry.auth.UpsertSessionTracker(s.serverCtx, tracker)
	if err != nil {
		return trail.FromGRPC(err)
	}
...
func (s *session) trackerAddParticipant(participant *party) error {
	...
	err := services.AddSessionTrackerParticipant(s.serverCtx, s.registry.auth, s.id.String(), &types.Participant{
		ID:         participant.user,
...
func (s *session) trackerRemoveParticipant(participantID string) error {
	...
	err := services.RemoveSessionTrackerParticipant(s.serverCtx, s.registry.auth, s.id.String(), participantID)
	return trace.Wrap(err)
}
...
func (s *session) trackerUpdateState(state types.SessionState) error {
	...
	err := services.UpdateSessionTrackerState(s.serverCtx, s.registry.auth, s.id.String(), state)
	return trace.Wrap(err)
}

func (s *session) trackerUpdateExpiry(expires time.Time) error {
	err := services.UpdateSessionTrackerExpiry(s.serverCtx, s.registry.auth, s.id.String(), expires)
	return trace.Wrap(err)
}
